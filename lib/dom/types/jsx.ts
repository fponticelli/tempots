import type { Property } from '@tempots/std/property'
import type { Effect } from '@tempots/std/types/functions'
import type { UnionToTuple } from '@tempots/std/types/tuples'
import { Scope } from '../core/scope'

// eslint-disable-next-line @typescript-eslint/no-namespace
export namespace JSX {
  export type Cancel = Effect<boolean>
  type Propertify<T, K extends keyof T, UT = UnionToTuple<T[K]>> = {
    [Index in keyof UT]:
      | Property<NonNullable<UT[Index]>>
      | Property<NonNullable<UT[Index]> | undefined>
      | Property<NonNullable<UT[Index]> | null>
  } extends Array<infer X>
    ? Exclude<
        X extends Property<true> | Property<false>
          ? Exclude<X, Property<true> | Property<false>> | Property<boolean>
          : X extends Property<null> | Property<undefined>
          ? Exclude<X, Property<null> | Property<undefined>>
          : X,
        Property<never>
      >
    : never
  export type Propertable<T> = {
    [K in keyof T]?: T[K] | Propertify<T, K>
  }

  export type HTMLAttributes<RefType extends EventTarget = EventTarget> =
    Propertable<BaseHTMLAttributes> &
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      TempoDOMAttributes<any, RefType> &
      DOMHandlers<RefType>
  export type SVGAttributes<RefType extends EventTarget = EventTarget> =
    Propertable<BaseSVGAttributes> &
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      TempoDOMAttributes<any, RefType> &
      DOMHandlers<RefType>
  ...
  export interface IntrinsicElements {
    ...
    path: SVGAttributes<SVGPathElement> & PathAttributes
  ...
  export type Render<T = any> = (scope: Scope) => undefined | Cancel
  ...
  export type TypedElement<T> =
    | Renderable<T>
    | Render<T>
    | Property<T>

  export type Element = TypedElement<any> | PrimitiveElement | Element[]
  ...
  export interface BaseSVGAttributes extends BaseHTMLAttributes {
  ...
  export type ComponentChildren = Element

  export interface TempoDOMAttributes<
    LifecycleValue,
    RefType extends EventTarget = EventTarget
  > extends LifecycleDOMAttributes<LifecycleValue, RefType> {
    children?: ComponentChildren
    style?:
      | string
      | CSSProperties
      | Property<string>
      | Propertable<CSSProperties>
  }

  export interface LifecycleDOMAttributes<
    LifecycleValue,
    RefType extends EventTarget = EventTarget
  > {
    afterMount?: (element: RefType) => LifecycleValue
  ...
  export interface DOMHandlers<Target extends EventTarget> {
    // Image Events
