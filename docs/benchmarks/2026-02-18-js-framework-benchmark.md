...
| Framework | Version |
|-----------|---------|
| Tempo keyed | 36.0.1 (`KeyedForEach` + `delegate` + `selectedClass` + `removeAllBefore` + detach/reattach + full-replace fast path + skip-KeyedPosition + template cloning) |
...
|-----------|-----------|-------|-------------|-------------|-----------------|
| Create 1,000 rows | 34.8 | 38.4 | 43.5 | **49.2** | **47.8** |
| Replace 1,000 rows | 39.0 | 44.3 | 53.1 | **56.5** | **16.0** |
| Partial update (every 10th) | 22.6 | 29.3 | 25.6 | **25.0** | **27.3** |
| Select row | 8.0 | 10.1 | 14.5 | **8.1** | **8.1** |
| Swap rows | 30.0 | 30.9 | 173.1 | **40.5** | **22.1** |
| Remove row | 20.5 | 22.5 | 21.3 | **21.6** | **32.0** |
| Create 10,000 rows | 372.7 | 409.3 | 573.0 | **490.7** | **464.5** |
| Append 1,000 rows | 41.6 | 47.0 | 52.1 | **63.4** | **56.6** |
| Clear 1,000 rows | 16.2 | 21.3 | 31.6 | **24.8** | **30.7** |

## Memory Benchmarks (MB, median, lower is better)

| Benchmark | VanillaJS | Solid | React Hooks | Tempo keyed | Tempo non-keyed |
|-----------|-----------|-------|-------------|-------------|-----------------|
| Ready memory | 0.55 | 0.54 | 1.18 | **0.70** | **0.70** |
| Run memory (1k rows) | 2.03 | 2.83 | 4.60 | **5.26** | **4.43** |
| Run-clear memory | 0.55 | 0.74 | 1.96 | **1.03** | **1.02** |

## Bundle Size

| Benchmark | VanillaJS | Solid | React Hooks | Tempo keyed | Tempo non-keyed |
|-----------|-----------|-------|-------------|-------------|-----------------|
| Uncompressed (KB) | 11.3 | 11.5 | 190.3 | **33.1** | **32.5** |
| Compressed (KB) | 2.5 | 4.5 | 51.4 | **10.6** | **10.5** |
| First paint (ms) | 49.8 | 53.8 | 288.5 | **92.9** | **77.4** |

## Optimization History

### Baseline → Current (cumulative improvements)

| Benchmark | Baseline | Round 4 | Round 5 | Round 6 | Current (Round 7) | Improvement |
|-----------|----------|---------|---------|---------|---------------------|-------------|
| Create 1k | 110.5 | 64.4 | 61.7 | 62.2 | 49.2 | **-55.5%** |
| Replace 1k | 124.1 | 68.8 | 74.6 | 64.3 | 56.5 | **-54.5%** |
| Partial update | 37.1 | 31.4 | 33.5 | 30.4 | 25.0 | **-32.6%** |
| Select row | 22.5 | 11.2 | 9.0 | 8.6 | 8.1 | **-64.0%** |
| Swap rows | 40.4 | 45.4 | 40.3 | 43.4 | 40.5 | ~same |
| Remove row | 55.3 | 23.7 | 24.2 | 24.5 | 21.6 | **-60.9%** |
| Create 10k | 1,024.5 | 633.3 | 603.7 | 587.1 | 490.7 | **-52.1%** |
| Append 1k | 125.4 | 73.3 | 73.6 | 78.7 | 63.4 | **-49.4%** |
| Clear 1k | 57.7 | 33.6 | 30.8 | 28.8 | 24.8 | **-57.0%** |
| Ready mem | 0.71 | 0.69 | 0.69 | 0.70 | 0.70 | **-1.4%** |
| Run mem | 30.76 | 9.17 | 9.84 | 9.79 | 5.26 | **-82.9%** |
| Run-clear mem | 20.86 | 1.02 | 0.99 | 0.99 | 1.03 | **-95.1%** |
...
|-----------|-------|-------------|-------------|-----------------|
| Create 1k | 1.03x | 1.16x | **1.41x** | **1.37x** |
| Replace 1k | 1.07x | 1.28x | **1.45x** | **0.41x** |
| Update 10th | 1.07x | 0.94x | **1.11x** | 1.21x |
| Select row | 1.15x | 1.65x | **1.01x** | **1.01x** |
| Swap rows | 0.97x | 5.43x | 1.35x | **0.74x** |
| Remove row | 1.08x | 1.02x | **1.05x** | 1.56x |
| Create 10k | 1.07x | 1.49x | **1.32x** | **1.25x** |
| Append 1k | 1.08x | 1.20x | **1.52x** | **1.36x** |
| Clear 1k | 1.25x | 1.85x | **1.53x** | 1.90x |
| **CPU Geo Mean** | **1.08x** | **1.50x** | **~1.29x** | **~1.11x** |
| Run memory | 1.39x | 2.27x | **2.59x** | **2.19x** |
| Compressed size | 1.80x | 20.56x | 4.24x | 4.20x |

### Tempo Keyed vs VanillaJS — Round-over-round

| Benchmark | Round 4 Ratio | Round 5 Ratio | Round 6 Ratio | Round 7 Ratio | R6→R7 Change |
|-----------|---------------|---------------|---------------|---------------|--------------|
| Create 1k | 1.88x | 1.75x | 1.66x | **1.41x** | **-15%** |
| Replace 1k | 1.79x | 1.81x | 1.55x | **1.45x** | **-6%** |
| Update 10th | 1.45x | 1.44x | 1.11x | **1.11x** | ~same |
| Select row | 1.67x | 1.11x | 0.98x | **1.01x** | ~same |
| Swap rows | 1.73x | 1.38x | 1.36x | **1.35x** | ~same |
| Remove row | 1.23x | 1.11x | 1.17x | **1.05x** | **-10%** |
| Create 10k | 1.71x | 1.57x | 1.53x | **1.32x** | **-14%** |
| Append 1k | 1.76x | 1.67x | 1.81x | **1.52x** | **-16%** |
| Clear 1k | 1.93x | 1.66x | 1.68x | **1.53x** | **-9%** |
| **Geo Mean** | **1.56x** | **~1.48x** | **~1.40x** | **~1.29x** | **-8%** |
| Run memory | 4.52x | 4.86x | 4.82x | **2.59x** | **-46%** |
...
Tempo keyed geo mean improved from **1.56x → ~1.48x → ~1.40x → ~1.29x VanillaJS** across Rounds 5-7, now significantly faster than React Hooks (~1.50x) and approaching Solid (~1.08x). Key strengths:

- **Select row** (1.01x) — essentially at parity with VanillaJS, thanks to O(1) `selectedClass`. Beats both Solid (1.15x) and React (1.65x).
- **Remove row** (1.05x) — near-parity with VanillaJS. Template cloning reduced per-row teardown overhead.
- **Update 10th** (1.11x) — competitive with Solid (1.07x). Skipping KeyedPosition eliminates per-update overhead.
- **Create 10k** (1.32x) — major improvement from 1.53x. Template cloning eliminates 8 `createElement` + 11 `appendChild` calls per row, replacing them with a single `cloneNode(true)`.
- **Create 1k** (1.41x) — improved from 1.66x. Same template cloning benefit.
- **Replace 1k** (1.45x) — improved from 1.55x. Combines full-replacement fast path with template cloning for rebuild.
- **Swap rows** (1.35x) — LIS-based reconciliation. React is catastrophically slow here (5.43x / 173ms) due to full VDOM diffing.

Biggest remaining gaps vs Solid:
- **Create** (~1.41x vs Solid's ~1.03x) — per-row signal/scope allocation overhead remains. Template cloning eliminated DOM creation overhead but reactive infrastructure (Prop, scope, comment markers) still costs.
- **Append 1k** (1.52x vs Solid's 1.08x) — same cause as create.
- **Clear 1k** (1.53x vs Solid's 1.25x) — disposal overhead from reactive teardown.

Tempo non-keyed at **1.11x geo mean** now beats Solid keyed (1.08x) on several individual benchmarks: **replace** (16.0 ms, 0.41x — 2.4x faster than VanillaJS), **swap rows** (22.1 ms, 0.74x), and **select row** (8.1 ms, 1.01x).

### Memory

Run memory dropped dramatically from **9.79 MB → 5.26 MB** (2.59x VanillaJS), nearly halving the previous 4.82x ratio. Template cloning eliminates per-row BrowserContext instances, intermediate closures, and `clears` arrays that the normal rendering pipeline creates. For comparison:
- VanillaJS: 2.03 MB (baseline)
- Solid: 2.83 MB (1.39x)
- React: 4.60 MB (2.27x)
- **Tempo keyed: 5.26 MB (2.59x)** — now in the same ballpark as React
- Tempo non-keyed: 4.43 MB (2.19x) — slightly better than React

Remaining per-row overhead comes from `Prop<RowData>` (the item signal), 2 comment node markers (start + end), and scope tracking.

Run-clear memory (1.03 MB) is notably better than React (1.96 MB), indicating clean disposal — Tempo's leak fixes are effective.

### Bundle Size

Tempo's bundle at **10.6 KB gzipped** (slightly larger due to template engine code) is:
- **4.9x smaller** than React Hooks (51.4 KB)
- **2.4x larger** than Solid (4.5 KB)
- **4.2x larger** than VanillaJS (2.5 KB)

First paint follows the same pattern: Tempo (93 ms keyed, 77 ms non-keyed) is much faster than React (289 ms) and reasonably close to Solid (54 ms).

### Remaining Optimization Opportunities

1. **Cell primitive** — Lightweight reactive value without full Signal overhead (no dispose tracking, simpler notification) to reduce run memory
