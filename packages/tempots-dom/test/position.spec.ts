...
import { ElementPosition, prop, signal } from "../src";
import { sleep } from "./helper";

describe("ElementPosition", () => {
  test("isFirst", () => {
    const pos = new ElementPosition(0, signal(3));
  ...
  test("even/odd", () => {
    let pos = new ElementPosition(0, signal(3));
    expect(pos.isEven).toStrictEqual(false);
    expect(pos.isOdd).toStrictEqual(true);

    pos = new ElementPosition(1, signal(3));
