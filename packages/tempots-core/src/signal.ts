...
export const effect = (
  ...
  // Wrap fn to snapshot dependency values and skip when unchanged.
  // This prevents effects from re-running when an intermediate computed
  // is marked dirty but recomputes to the same value.
  const wrappedFn = actualFn
  const len = signals.length
  const prev: unknown[] = new Array(len)
  let initialized = false
  const guardedFn = () => {
    let changed = !initialized
    for (let i = 0; i < len; i++) {
      const v = signals[i].get()
      if (v !== prev[i]) {
        prev[i] = v
        changed = true
      }
    ...
    initialized = true
    if (changed) wrappedFn()
  }
  const signal = computed(guardedFn, signals)
  const clear = () => {
    signal.dispose()
    if (options.abortSignal != null) {
      options.abortSignal.removeEventListener('abort', clear)
