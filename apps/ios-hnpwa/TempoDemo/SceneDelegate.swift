    ...
    func scene(
        ...
        if let bundlePath = Bundle.main.path(forResource: "bundle", ofType: "js"),
           let jsCode = try? String(contentsOfFile: bundlePath, encoding: .utf8) {
            print("[TempoHNPWA] Evaluating JS bundle (\(jsCode.count) bytes)")
            bridge.jsContext.evaluateScript(jsCode)
            bridge.flushMicrotasks()
            print("[TempoHNPWA] JS bundle evaluated")
        } else {
            print("[TempoHNPWA] WARNING: Could not load dist/bundle.js")
        }
