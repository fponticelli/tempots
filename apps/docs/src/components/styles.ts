export const Styles = {
  sidebar: {
    heading: 'text-xs font-semibold leading-6 text-gray-600',
  },
  heading: {
    small: 'font-bold text-lg',
    large: 'font-bold text-2xl text-center py-2',
    subSmall: 'font-semibold text-sm text-gray-600',
  },
  block: {
    code: 'w-full h-full overflow-auto font-mono text-sm bg-gray-100 text-gray-700 p-2 rounded',
  },
  input: {
    focus:
      'focus:outline-none focus:ring focus:ring-offset-2 focus:ring-blue-500 focus:ring-opacity-90',
  },
  icon: {
    bordered:
      'flex h-6 w-8 shrink-0 items-center justify-center rounded border border-gray-400 bg-white text-[0.625rem] font-medium text-gray-600 group-hover:border-blue-600 group-hover:text-blue-600',
  },
  prose:
    'prose prose-stone prose-md prose-h1:text-3xl prose-h2:text-2xl prose-h3:text-xl prose-h4:text-lg prose-h5:text-lg prose-h6:text-lg prose-code:bg-gray-200 prose-code:rounded prose-code:before:content-[""] prose-code:after:content-[""] prose-code:px-1 prose-code:py-0.5 prose-code:font-medium',
  //  dark:prose-invert
}
